...
这是一个 Windows 离线桌面应用，用于非标机械设计日常选型计算。第一版目标不是只做高频项，而是覆盖根目录 PDF `非标笔记 2025-6-20 18396 1(1).pdf` 中全部 23 章选型内容。

核心要求：
- 计算输出必须包含过程分析、结论、风险和来源页码。
- 安全系数必须由用户手动输入或确认，系统不能自动代入。
- 厂家样本以 PDF 为主，后续允许 Excel/CSV 辅助导入。
- 厂家型号推荐只能基于已导入且启用的样本库。
- 报告导出目标为 PDF 和 Excel。
...
当前 `main` 与 `origin/main` 同步，HEAD 为：
- `5614101 (HEAD -> main, origin/main) feat: add root pdf ingest and knowledge search`

当前工作区有未提交的 Phase 3 代码，尚未 code-review，尚未 commit，尚未 push。

另外根目录有一个未跟踪文件：
- `非标笔记 2025-6-20 18396 1(1).docx`

该 docx 未确认用途，接手时不要擅自删除或提交，先询问或确认来源。
