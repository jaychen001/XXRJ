...
- `vendor/recommendation` 与结果页型号推荐：补充厂家字段别名、单位兼容检查、旋转气缸等部件类型推断；计算结果页可匹配型号、查看满足/不足条件，并把最终选择传入报告导出。
- 结果页型号推荐区补齐样本导入确认链：本地文件路径读取、抽取预览、字段映射确认、确认导入、已导入样本库启用/停用/删除；修正后端确认导入时未应用目标字段映射的问题。
...
**交付内容**：
- 实现厂家 PDF 样本导入：文本/表格抽取、样本位置、抽取置信度、字段映射、人工确认。
- 实现 Excel/CSV 辅助导入：表头读取、字段映射、单位归一、失败行报告。
- 实现型号推荐：按计算结果页的需求参数、样本库字段、启用状态和用户过滤条件筛选候选型号。
- 实现样本库版本管理：导入记录、禁用、删除、重复型号处理；第一版主界面不暴露独立样本库入口。
...
- `selector-desktop/src/features/calculation/RecommendationPanel.tsx` — 计算结果内候选型号结果面板。
- `selector-desktop/src/features/calculation/VendorImportPanel.tsx` — 计算结果内样本导入、预览和字段映射确认。
- `selector-desktop/src/features/calculation/vendor-import-options.ts` — 样本字段选项、格式识别和模块类型推断。
- `selector-desktop/src/features/calculation/VendorLibraryPanel.tsx` — 计算结果内样本库启用、停用和删除。
- `selector-desktop/src/features/reports/ReportExportDialog.tsx` — 导出当前计算报告时携带候选型号和最终选择。

**验收标准**：
- 文本型厂家 PDF 能进入抽取预览，显示疑似型号参数、样本位置、置信度和字段映射。
- 用户可在结果页通过系统文件选择窗口选择 PDF、Excel、CSV、TSV 样本，仍可手动填写路径。
- 未确认字段映射时不能写入型号库。
...
**交付内容**：
- 实现 PDF 和 Excel 报告导出，报告包含输入、过程、结论、风险、候选型号和最终选择型号，不展示内部资料来源。
- 保留开发侧回归验证，确保既定选型目录的计算和规则模块无缺口。
- 保留开发侧一键公式/规则样例回归，不作为用户主界面入口。
...
- `selector-desktop/src/features/reports/ReportExportDialog.tsx` — 计算结果页内报告导出区域。
- `selector-desktop/src-tauri/tauri.conf.json` — 打包、权限、外部资源配置。
- `selector-desktop/scripts/package-windows.cmd` — Windows 打包与产物检查入口。
- `selector-desktop/scripts/validate-windows-package.ps1` — 安装包哈希输出和隐私扫描。
- `selector-desktop/scripts/smoke-release-app.ps1` — 使用隔离数据目录启动 release exe 并验证数据库初始化。

**验收标准**：
- 用户能在计算结果页选择 PDF 或 Excel，并通过系统保存窗口或手动路径导出报告。
- 报告包含输入、过程、结论、风险、候选型号和最终选择型号，不展示内部资料来源。
- 开发侧覆盖检查和回归样例保留在内部命令中，不作为用户主界面入口。
- Windows 打包产物能生成 MSI/NSIS 安装包，脚本输出 SHA256 并确认包内没有数据库、环境文件、密钥和开发路径。
- 发布冒烟测试通过 `SELECTOR_DESKTOP_DATA_DIR` 使用独立测试数据目录，不写正常 `%APPDATA%`。
