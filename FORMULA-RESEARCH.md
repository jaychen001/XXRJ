...
1. 厂家样本库导入确认流程已接入结果页：支持系统文件选择窗口和手动路径输入，PDF/Excel/CSV/TSV 预览、字段映射人工确认、启用/停用和删除都在型号推荐区域内完成，第一版主界面不单独暴露样本库页面。
2. 型号推荐规则已接入结果页：按计算需求参数匹配已启用样本库，输出满足条件、不足原因和最终型号选择，报告导出可带上候选与最终选择。
3. 后续继续补充真实厂家样本字段别名，重点覆盖伺服/步进、气缸、旋转气缸、直线导轨、滚珠丝杠和减速机常见表头。
